...
// 화면·모델용 한글 라벨
const PART_LABEL_KO = {
  head: '머리', eye: '눈', ear: '귀', neck: '목', nose: '코', chest: '가슴',
  abdomen: '배', waist: '허리', knee: '무릎', skin: '피부',
  face_neck: '눈·귀·코·목', back_joint: '허리·관절', other: '그 외', multiple: '여러 군데',
};
const FIELD_LABEL = {
...
function stateLine(state, filled) {
  const parts = filled.map((f) => {
    const v = state[f];
    const shown = Array.isArray(v) ? v.join('/') : f === 'body_part' ? partLabel(v) : v;
...
export async function POST(req) {
  ...
  choice = await runToolLoop(choice, messages, events, loop);
  ...
  const askedField = body.state && typeof body.state.asked_field === 'string' ? body.state.asked_field : null;
  const mergedState = parsed ? mergeState(state, parsed.state, askedField, user_text) : { ...state };
  ...
  if (!allFilled) {
    ...
    const useModel = !!(parsed && parsed.next_field && parsed.question && !repeated);
    const nextField = useModel ? parsed.next_field : sum2.next;
    let question = useModel ? parsed.question : `${FIELD_LABEL[nextField]}은 어떻게 돼요? 조금만 더 알려 주세요.`;
    if (repeated) {
      events.push({
        event: 'review',
  ...
  let modelQuestions = parsed && parsed.questions.length > 0 ? parsed.questions.slice(0, 3) : null;
  if (!modelScript || !modelQuestions || modelQuestions.length < 3) {
    try {
      ...
      finalChoice = await runToolLoop(finalChoice, finalMessages, events, loop);
      toolCallCount = loop.toolCallCount;
      toolRound = loop.toolRound;
      const finalParsed = parseSolarReply(finalChoice);
      if (finalParsed && finalParsed.script.length > 0) modelScript = finalParsed.script;
      if (finalParsed && finalParsed.questions.length >= 3) modelQuestions = finalParsed.questions.slice(0, 3);
      else if (finalParsed && finalParsed.questions.length > 0 && !modelQuestions) modelQuestions = finalParsed.questions;
      events.push({
        event: 'review',
        line: finalParsed
      ...
      events.push({
        event: 'review',
        line: '마무리 호출 실패: ' + (err && err.message ? err.message : String(err)),
        body: null, hint: null, tool: null, tool_status: null, tool_error: null, payload: {},
      });
    }
  } else {
  ...
  while (loop.toolRound < 8) {
    ...
    for (const tc of calls) {
      ...
      events.push({
        event: 'tool_call',
        line: `${label} 호출`,
        body: null, hint: null, tool: fnName, tool_status: 'called', tool_error: null,
        payload: { args },
      });

      let toolRes;
      if (typeof TOOL_MAP[fnName] !== 'function') {
      ...
      events.push({
        ...
        body: null, hint: null, tool: fnName,
    ...
    try {
      ...
      events.push({
        event: 'review',
        line: 'Solar 재호출 실패: ' + (err && err.message ? err.message : String(err)),
        body: null, hint: null, tool: null, tool_status: null, tool_error: null, payload: {},
      });
      break;
    }
  ...
  return [
    { role: 'system', content: SKILL_CONTENT },
    {
      ...
      content: [
        ...
        meds.length >= 2 ? '복용 약이 2개 이상이니 먼저 툴로 DUR 병용금기와 효능군 중복을 확인하고, 등록된 사실이 있으면 대본에 "등록돼 있어 의사에게 확인하겠다"는 한 문장으로만 넣어. 위험하다거나 끊으라는 판단은 하지 마.' : null,
...
function mergeState(existing, modelState, askedField, userText) {
  ...
  const negated = /잘\s*모르|모르겠|몰라|기억\s*안|없어|없음|안\s*먹|안\s*했|아직 아무/.test(userText || '');
  for (const key of STATE_FIELDS) {
    ...
    if (/^(없음|없어|기억 안 남|모름)$/.test(noneLike) && key !== askedField && !negated) continue;
    if (key === 'current_meds' || key === 'tried_things') {
      const arr = Array.isArray(mv) ? mv : [mv];
      const cleaned = arr.map((x) => String(x).trim()).filter(Boolean);
      if (cleaned.length > 0) merged[key] = cleaned;
    } else {
      const s = Array.isArray(mv) ? mv.map(String).join(', ').trim() : String(mv).trim();
      if (s) merged[key] = s;
    }
  }
...
function fillAskedField(state, askedField, text) {
  ...
  if (!t) return;
  let value = t;
  if (/잘\s*모르|모르겠|몰라|기억\s*안|없어|없음|안\s*먹|안\s*했|아직 아무/.test(t)) {
    value = askedField === 'since_when' ? '기억 안 남' : '없음';
  }
  if (askedField === 'body_part') value = normalizeBodyPart(value);
  state[askedField] = (askedField === 'current_meds' || askedField === 'tried_things') ? [value] : value;
}
...
function buildAgentMessages(body, state) {
  ...
  const asked = body.state && body.state.asked_field ? FIELD_LABEL[body.state.asked_field] || body.state.asked_field : null;
  return [
    { role: 'system', content: SKILL_CONTENT },
    {
      role: 'user',
      content: [
        ...
        `[사용자가 방금 한 말]\n${body.user_text}`,
        '',
        "SKILL.md의 '응답 형식' 절에 적힌 JSON 객체 하나만 출력해. 그 외 글자는 쓰지 마.",
      ].filter((l) => l !== null).join('\n'),
    },
  ];
}
...
export function parseSolarReply(choice) {
  ...
  if (typeof msg.content === 'string') {
    ...
    for (const block of msg.content) {
      ...
      if (t && String(t).trim()) { content = String(t); break; }
    }
  }
  ...
  const rawState = parsed.state && typeof parsed.state === 'object' && !Array.isArray(parsed.state) ? parsed.state : {};
  const state = {};
  for (const k of STATE_FIELDS) {
    ...
    if (k === 'current_meds' || k === 'tried_things') {
      ...
      const s = Array.isArray(v) ? v.map(String).join(', ').trim() : String(v).trim();
      if (s) state[k] = s;
    }
  }

  const nextField = STATE_FIELDS.includes(parsed.next_field) ? parsed.next_field : null;
  ...
  const chips = Array.isArray(parsed.chips)
    ? parsed.chips.map((c) => {
        if (typeof c === 'string') return c.trim() ? { id: c.trim(), label: c.trim() } : null;
        if (!c || typeof c !== 'object') return null;
        const label = String(c.label ?? c.id ?? '').trim();
        if (!label) return null;
        return { id: String(c.id ?? label), label, isEscape: c.isEscape === true, isBodyPart: c.isBodyPart === true };
      }).filter(Boolean)
    : [];
  const script = Array.isArray(parsed.script) ? parsed.script.map((s) => String(s).trim()).filter(Boolean) : [];
...
function buildChips(modelChips, nextField) {
  ...
  for (const c of modelChips) {
    ...
    out.push({
      id: c.id || c.label,
      label: c.label,
      isEscape: c.isEscape === true || /잘\s*모르|모르겠|기억\s*안|^없어$|^없음$/.test(c.label),
      isBodyPart: nextField === 'body_part' ? (c.isBodyPart !== false) : (c.isBodyPart === true),
    });
  }
  // 탈출 칩은 맨 뒤 하나만
  ...
  if (escapes.length > 1) {
    ...
    for (let i = out.length - 1; i >= 0; i -= 1) if (out[i].isEscape && out[i] !== keep) out.splice(i, 1);
  }
  ...
  if (out.length < 4) {
    const defs = defaultChips(nextField).filter(
      (d) => d.id !== 'escape' && !out.some((o) => o.id === d.id || o.label === d.label)
    );
...
function buildFallbackScript(state) {
  const parts = [];
  if (state.since_when) parts.push(`${state.since_when}부터 ${partLabel(state.body_part)} 쪽이 불편했어요.`);
  else if (state.body_part) parts.push(`${partLabel(state.body_part)} 쪽이 불편해요.`);
  if (state.symptom_desc) parts.push(`증상은 ${state.symptom_desc} 느낌이에요.`);
  if (state.current_meds.length > 0) parts.push(`지금 먹는 약은 ${state.current_meds.join(', ')}이에요.`);
  if (state.tried_things.length > 0) parts.push(`그동안 해 본 것은 ${state.tried_things.join(', ')}이에요.`);
...
function buildFallbackQuestions(state) {
  ...
  if (state.tried_things.length > 0 && state.tried_things[0] !== '없음') qs.push('해 본 것들이 효과가 없었는데 다른 원인일 수 있나요?');
  qs.push('검사가 필요한가요?');
  return qs.slice(0, 3);
}
